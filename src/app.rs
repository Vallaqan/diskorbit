            ...
            match rx.try_recv() {
                ...
                Ok(ScanMsg::Done(mut node)) => {
                    if self.total_bytes > 0 {
                        node.percentage = self.used_bytes as f32 / self.total_bytes as f32 * 100.0;
                    }
                    self.footer = format!(
                        "{}  ·  {} in {} items",
                        self.drives
                ...
                ui.horizontal(|ui: &mut egui::Ui| {
                    ...
                    ui.vertical(|ui: &mut egui::Ui| {
                        ...
                        ui.horizontal(|ui: &mut egui::Ui| {
                            ...
                            ui.label(
                                RichText::new("ORBIT")
                                    .color(Color32::from_rgb(0xFB, 0x92, 0x3C))
                                    .font(bold(20.0)),
                            );
                        });
                        ui.label(
                            RichText::new("STORAGE ANALYSIS SYSTEM")
                                .color(TEXT_DIM)
                                .font(mono(8.0)),
                        );
                    });
                        ...
                        ui.horizontal(|ui: &mut egui::Ui| {
                            ...
                            if self.total_bytes > 0 {
                                ui.with_layout(
                                    ...
                                    |ui: &mut egui::Ui| {
                                        ui.label(
                                            RichText::new(fmt_bytes(self.total_bytes))
                                                .color(TEXT_PRIMARY)
                                                .font(mono(11.0)),
                                        );
                                        ui.label(
                                        ...
                                        ui.add_space(8.0);
                                        ui.label(
                                            RichText::new(fmt_bytes(self.free_bytes))
                                                .color(GREEN)
                                                .font(mono(11.0)),
                                        );
                                        ui.label(
                                            RichText::new("FREE ").color(TEXT_DIM).font(mono(11.0)),
                                        );
                                        ui.add_space(8.0);
                                        ui.label(
                                            RichText::new(fmt_bytes(self.used_bytes))
                                                .color(ACCENT)
                                                .font(mono(11.0)),
                                        );
                                        ui.label(
                                            RichText::new("USED ").color(TEXT_DIM).font(mono(11.0)),
                                        );
                                    },
                                );
                            }
    ...
    fn show_column_headers(&self, ui: &mut egui::Ui) {
        egui::Frame::none()
            ...
            .show(ui, |ui: &mut egui::Ui| {
                ...
                let (rect, _) =
                    ui.allocate_exact_size(Vec2::new(avail, 18.0), egui::Sense::hover());
                let p = ui.painter();
                ...
                p.text(
                    egui::pos2(rect.min.x + bar_right, cy),
                ...
                p.text(
                    egui::pos2(rect.min.x + size_right, cy),
                    egui::Align2::RIGHT_CENTER,
                    "SIZE",
                    mono(11.0),
                    TEXT_MUTED,
                ...
                p.text(
                    egui::pos2(rect.min.x + pct_right, cy),
                    egui::Align2::RIGHT_CENTER,
                    "%",
                    mono(11.0),
                    TEXT_MUTED,
        ...
        egui::ScrollArea::vertical()
            ...
            .show(ui, |ui: &mut egui::Ui| {
                ...
                if let Some(root) = self.scan_result.take() {
                    ...
                    ui.vertical_centered(|ui: &mut egui::Ui| {
                        ui.label(
                            RichText::new("Select a drive and click Scan")
                                .color(TEXT_DIM)
                                .font(mono(13.0)),
                        );
                    });
...
fn draw_node(
    ...
    // Usage bar
    let bar_rect = egui::Rect::from_min_size(egui::pos2(bar_left, cy - 2.5), Vec2::new(BAR_W, 5.0));
    ...
    if resp.clicked() && !node.children.is_empty() {
        if is_expanded {
            expanded.remove(&node.full_path);
        } else {
            expanded.insert(node.full_path.clone());
        }
    }
...
fn clamp_text(s: &str, max_px: f32, ui: &egui::Ui, font: FontId) -> String {
    ...
    let w = fonts
        .layout_no_wrap(s.to_string(), font.clone(), TEXT_PRIMARY)
        .size()
        .x;
    if w <= max_px {
        return s.to_string();
...
fn drive_usage(root: &str) -> Option<(u64, u64, u64)> {
    #[cfg(target_os = "windows")]
    {
        ...
        #[link(name = "kernel32")]
        unsafe extern "system" {
            fn GetDiskFreeSpaceExW(d: *const u16, fc: *mut u64, tot: *mut u64, ft: *mut u64)
                -> i32;
        }
...
mod tests {
    ...
    fn count_items_flat_directory() {
        let dir = make_dir(
            "root",
            vec![make_leaf("a.txt", 100), make_leaf("b.txt", 200)],
        );
    ...
    fn available_drives_nonempty() {
        ...
        assert!(
            !drives.is_empty(),
            "should always have at least one drive/root"
        );
    }
}
